...
(function() {
    const delay = 90000; // 30 секунд
    const now = Date.now();
    let executionTime = localStorage.getItem('executionTime');

    if (!executionTime) {
        // Если зашли первый раз — фиксируем время гибели сайта
        executionTime = now + delay;
        localStorage.setItem('executionTime', executionTime);
        console.log('perviz ray')
    } else {
        // Преобразуем строку из localStorage в число
    ...
    console.log({now, executionTime, diff: executionTime - now});
    if (now >= executionTime) {
        // Если время вышло — блокируем моментально
        console.log('blok');
        blockSite();
    } else {
        // Если время еще есть — ставим таймер на остаток
        setTimeout(blockSite, executionTime - now);
    }
})();
